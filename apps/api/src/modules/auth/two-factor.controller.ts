...
export class TwoFactorController {
  ...
  async setup(@CurrentUser() user: User) {
    ...
    const setup = await this.twoFactorService.generateSetup(user.id, user.email);
  ...
  async enable(
    ...
    return {
      ...
      warning: 'Guarda estos códigos de respaldo en un lugar seguro. Los necesitarás si pierdes acceso a tu aplicación de autenticación.',
    };
  }
  ...
  getBackupCodesCount(@CurrentUser() user: User) {
    const remaining = this.twoFactorService.getRemainingBackupCodesCount(user.id);

    return {
      remaining,
      total: 8,
      warning: remaining <= 2 ? 'Te quedan pocos códigos de respaldo. Considera regenerarlos.' : null,
    };
  }
}
