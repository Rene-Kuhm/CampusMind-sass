...
import { ILlmProvider, LlmProviderType, LLM_PROVIDERS } from '../interfaces/rag.interface';
...
export class LlmProviderFactory {
  ...
  static createProvider(
    ...
    const type = providerType || config.get<LlmProviderType>('LLM_PROVIDER', 'openai');
   ...
   */
  static isProviderConfigured(config: ConfigService, providerType: LlmProviderType): boolean {
  ...
  static getConfiguredProviders(config: ConfigService): LlmProviderType[] {
    const providers: LlmProviderType[] = ['openai', 'gemini', 'deepseek', 'groq'];
  ...
  private static createOpenAI(http: HttpService, config: ConfigService, modelOverride?: string): OpenAIProvider {
    const apiKey = config.get<string>('OPENAI_API_KEY', '');
    const model = modelOverride || config.get<string>('OPENAI_MODEL', LLM_PROVIDERS.openai.defaultModel);
    return new OpenAIProvider(http, apiKey, model);
  }

  private static createGemini(http: HttpService, config: ConfigService, modelOverride?: string): GeminiProvider {
    const apiKey = config.get<string>('GEMINI_API_KEY', '');
    const model = modelOverride || config.get<string>('GEMINI_MODEL', LLM_PROVIDERS.gemini.defaultModel);
    return new GeminiProvider(http, apiKey, model);
  }

  private static createDeepSeek(http: HttpService, config: ConfigService, modelOverride?: string): DeepSeekProvider {
    const apiKey = config.get<string>('DEEPSEEK_API_KEY', '');
    const model = modelOverride || config.get<string>('DEEPSEEK_MODEL', LLM_PROVIDERS.deepseek.defaultModel);
    return new DeepSeekProvider(http, apiKey, model);
  }

  private static createGroq(http: HttpService, config: ConfigService, modelOverride?: string): GroqProvider {
    const apiKey = config.get<string>('GROQ_API_KEY', '');
    const model = modelOverride || config.get<string>('GROQ_MODEL', LLM_PROVIDERS.groq.defaultModel);
