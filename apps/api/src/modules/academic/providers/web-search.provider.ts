...
export class WebSearchProvider {
  ...
  private determineResourceType(
    ...
    if (content.includes("video") || content.includes("youtube"))
      return "video";
    if (content.includes("course") || content.includes("curso"))
      return "course";
    if (content.includes("book") || content.includes("libro")) return "book";
    if (content.includes("tutorial")) return "notes";
    if (content.includes("paper") || content.includes("research"))
      return "paper";
  ...
  private isEducationalResource(url: string, title: string): boolean {
    ...
    return (
      educationalDomains.some((domain) => lowercaseUrl.includes(domain)) ||
      url.endsWith(".pdf") ||
