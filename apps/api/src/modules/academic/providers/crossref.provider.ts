...
export class CrossrefProvider implements AcademicProvider {
  ...
  constructor(
    ...
    this.email = this.config.get<string>('CROSSREF_EMAIL') ||
  ...
  async getById(externalId: string): Promise<AcademicResource | null> {
    try {
      ...
      const doi = externalId.startsWith('10.') ? externalId : `10.${externalId}`;
  ...
  private buildSearchParams(query: SearchQuery): string {
    ...
    params.set('select', 'DOI,title,author,abstract,published,published-online,published-print,type,subject,is-referenced-by-count,references-count,URL,link,license,container-title,publisher');
  ...
  private extractPublicationDate(work: CrossrefWork): { full?: string; year?: number } | undefined {
    const dateParts =
