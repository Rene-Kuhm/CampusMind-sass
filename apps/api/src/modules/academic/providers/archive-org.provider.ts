...
export const CURATED_TEXTBOOKS: Record<string, CuratedTextbook[]> = {
  ...
  biomechanics: [
    {
      identifier:
        "382169905kapandjithephysiologyofthejointsvolume1theupperlimb",
    ...
    {
      identifier:
        "kapandjithephysiologyofthejointsvolume3thevertebralcolumnpelvicgirdleandhead2008",
  ...
  programming: [
    {
      identifier:
        "fundamentos-de-programacion-4ta-edicion-luis-joyanes-aguilar",
...
export class ArchiveOrgProvider {
  ...
  async search(query: SearchQuery): Promise<SearchResult> {
    ...
    try {
      ...
      const mediaTypeQuery = mediaTypes
        .map((t) => `mediatype:${t}`)
        .join(" OR ");
      ...
      const params = new URLSearchParams({
        ...
        page: String(page),
        "fl[]":
          "identifier,title,creator,description,date,mediatype,downloads,subject,language",
      });
  ...
  private mapToAcademicResource(doc: any): AcademicResource {
    ...
    const pdfUrl = isBook
      ? `${this.baseUrl}/download/${identifier}/${identifier}.pdf`
      : null;
   ...
   */
  async getCuratedTextbooks(
    category: TextbookCategory = "kinesiology",
  ): Promise<AcademicResource[]> {
    const textbooks =
      CURATED_TEXTBOOKS[category] || CURATED_TEXTBOOKS.kinesiology;
